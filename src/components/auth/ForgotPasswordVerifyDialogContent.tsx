...
const ForgotPasswordVerifyDialogContent = ({
  setState,
}: {
  onBack?: () => void;
  ...
  const { email, setToken } = useStateContext();
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/forgot-verify-otp`,
        {
          email: email,
          otp: data.otp,
        },
      ...
      <div className="flex flex-col gap-5 sm:gap-6 lg:gap-8 items-center font-poppins">
        ...
        <div className="flex flex-col gap-2 sm:gap-3 lg:gap-4 items-start w-full">
          ...
          <p className="text-[#637381] text-sm sm:text-base leading-6">
            ...
            <span className="text-[#212B36] font-medium break-all">
              {email}
            </span>
            ...
            {countdown > 0 ? (
              ...
              <>
                ...
                <button
                  ...
                  className="text-[#122464] font-semibold hover:underline cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
                >
...
export default ForgotPasswordVerifyDialogContent;
